...
import { toProject, type Project, type ProjectRow, type ProjectStatus } from "../models/project.js";
...
const getProjectById = async (db: Connection, id: number): Promise<Project | null> => {
	const [rows] = await db
		.promise()
		.query<ProjectRowPacket[]>(
			"SELECT id, owner_user_id, name, description, start_date, due_date, budget, status, created_at, updated_at FROM projects WHERE id = ? LIMIT 1",
			[id]
		);
...
export const createProjectController = (db: Connection) => {
	const createProject = async (req: Request, res: Response) => {
		try {
			...
			if (ownerUserIdFromAuth && ownerUserIdFromBody && ownerUserIdFromAuth !== ownerUserIdFromBody) {
				res.status(403).json({ message: "ownerUserId does not match authenticated user" });
				return;
			}

			if (!ownerUserId || !name) {
				badRequest(req, res, "name is required (ownerUserId comes from auth token)" );
				return;
			}
			...
			const [result] = await db
				...
				.execute<ResultSetHeader>(
					...
					[ownerUserId, name, description, startDate, dueDate, budget, status]
				);
...
export const getAllProjectsController = (db: Connection) => {
	const getAllProjects = async (req: Request, res: Response) => {
		try {
			const [rows] = await db.promise().query<ProjectRowPacket[]>("SELECT * FROM projects");
